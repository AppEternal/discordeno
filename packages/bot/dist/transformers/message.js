...
export function transformMessage(bot, payload, extra) {
  ...
  if (props.referencedMessage && payload.referenced_message)
    message.referencedMessage = bot.transformers.message(bot, payload.referenced_message, {
      shardId: extra?.shardId,
    })
  if (props.messageSnapshots && payload.message_snapshots)
    message.messageSnapshots = payload.message_snapshots.map((snap) =>
      bot.transformers.messageSnapshot(bot, snap, {
        shardId: extra?.shardId,
      }),
    )
  ...
  if (props.thread && payload.thread)
    message.thread = bot.transformers.channel(bot, payload.thread, {
      guildId,
    })
...
export function transformMessagePin(bot, payload, extra) {
  ...
  if (props.message && payload.message)
    messagePin.message = bot.transformers.message(bot, payload.message, {
      shardId: extra?.shardId,
    })
