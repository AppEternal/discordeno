  ...
  const bucket = {
    ...
    processWaiting: async function () {
      ...
      while (bucket.waiting.length > 0) {
        bucket.logger.info(
          `[InvalidBucket] processing waiting queue while loop ran with ${bucket.waiting.length} pending requests to be made. ${JSON.stringify(bucket)}`,
        )
        if (!bucket.isRequestAllowed() && bucket.resetAt !== undefined) {
          bucket.logger.warn(
            `[InvalidBucket] processing waiting queue is now paused until more requests are available. ${bucket.waiting.length} pending requests. ${JSON.stringify(bucket)}`,
          )
