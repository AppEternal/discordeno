...
export function createRestManager(options) {
  ...
  const rest = {
    ...
    /** Processes the rate limit headers and determines if it needs to be rate limited and returns the bucket id if available */ processHeaders(
      url,
      headers,
      identifier,
    ) {
    ...
    simplifyUrl(url, method) {
      const routeInformationKey = [method]
